 ...
 *     description: >
 *       Soft-deletes a user from the project by setting removed_at timestamp.
 *       Historical data (time logs, tasks) is preserved for reporting purposes.
...
export async function DELETE(request: NextRequest, context: RouteContext) {
  try {
    ...
    // Preveri da je član aktiven (removed_at IS NULL)
    ...
    // Soft delete — nastavi removed_at namesto fizičnega brisanja
    ...
    // Poišči vse user_story id-je tega projekta
    const { data: storyIds } = await supabaseAdmin
      .from("user_stories")
      .select("id")
      .eq("project_id", projectId)
      .is("deleted_at", null);
    ...
    if (storyIds && storyIds.length > 0) {
      await supabaseAdmin
        ...
        .is("deleted_at", null)
        .neq("status", "completed"); // zaključenih taskov ne resetiramo
    }
 ...
 *     description: >
 *       Updates the project role of an active member.
