...
export default function BacklogPage() {
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        setAssignError(data.error || "Failed to assign stories.");
        return;
      }
      setSelectedIds([]);
      await loadBacklog();
  ...
  const assignedPoints = assigned.reduce(
    ...
    <div className="p-6">
              ...
              <button
                ...
                title={
                  !activeSprint
                    ? "No active sprint"
                    : exceedsVelocity
                      ? `Exceeds sprint velocity (${selectedPoints}/${remainingVelocity} pts remaining)`
                      : undefined
                }
      ...
      {activeSprint ? (
            ...
            {activeSprint.velocity != null && (
              <span>
                ...
                <span className="text-foreground font-medium">
                  {assignedPoints}/{activeSprint.velocity} pts
                </span>
              </span>
            )}
        ...
        <div className="flex items-center gap-2.5 px-4 py-2.5 rounded-xl border border-border bg-surface mb-5 text-sm text-muted">
          ...
          No active sprint — stories cannot be assigned until a sprint is
          active.
        </div>
      )}
      ...
      {currentList.length > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-sm text-subtle">
            {activeTab === "unassigned"
              ? "All stories are assigned to a sprint or completed."
              : activeTab === "assigned"
                ? activeSprint
