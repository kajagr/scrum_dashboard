...
const STATUS_CONFIG: Record<string, { label: string; pill: string; order: number }> = {
  ...
  in_progress: { label: "In Progress", pill: "bg-accent-light text-accent-text border border-accent-border", order: 1 },
  ready: { label: "Ready", pill: "bg-primary-light text-primary border border-primary-border", order: 2 },
...
export default function SprintBoardPage() {
  ...
  const handleMarkReady = async (story: Story) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        setActionError((prev) => ({ ...prev, [story.id]: data.error || "Error marking story as ready." }));
        return;
      }
      await fetchData();
    } catch {
  ...
  const openEdit = async (task: TaskWithAssignee) => {
    ...
    finally { setEditLoadingMembers(false); }
  ...
  const saveEdit = async () => {
    ...
    if (hours !== null && (isNaN(hours) || hours <= 0)) { setEditError("Estimated hours must be a positive number."); return; }
    setEditSaving(true);
    setEditError(null);
    try {
      const res = await fetch(`/api/tasks/${editingTask.id}`, {
        ...
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({ action: "edit", description: editDescription.trim() || null, estimated_hours: hours, assignee_id: editAssigneeId || null }),
      });
      const data = await res.json();
      if (!res.ok) { setEditError(data.error ?? "Error saving task."); return; }
      setEditingTask(null);
      await fetchTasks(stories.map((s) => s.id));
    } catch { setEditError("A server error occurred."); }
  ...
  const deleteTask = async (taskId: string) => {
    ...
    try {
      ...
      if (res.ok) { await fetchTasks(stories.map((s) => s.id)); }
      else { const data = await res.json(); alert(data.error || "Error deleting task."); }
    } catch { alert("Error deleting task."); }
  };

  const totalSprintEstimated = Object.values(tasksByStory).flat().reduce((sum, t) => sum + (t.estimated_hours ?? 0), 0);
  const totalSprintLogged = Object.values(tasksByStory).flat().reduce((sum, t) => sum + (t.logged_hours ?? 0), 0);
  const totalSprintRemaining = Math.max(0, totalSprintEstimated - totalSprintLogged);
    ...
    <div className="p-6">
        ...
        <>
          ...
          <div className="flex items-center gap-4 p-4 rounded-xl border border-primary-border bg-primary-light mb-6 flex-wrap">
            ...
            <div className="ml-auto flex items-center gap-4 text-xs text-muted">
              ...
              {activeSprint.velocity && <span>Velocity: <span className="font-semibold text-foreground">{activeSprint.velocity}</span></span>}
            </div>
          </div>
          ...
          {totalTasks > 0 && (
            <div className="mb-6">
              <div className="flex justify-between items-center text-xs text-muted mb-1.5">
                ...
                {showHours ? (
                  <span>
                    <span className="text-foreground font-semibold">{totalSprintLogged}h</span> logged ·{" "}
                    <span className="text-foreground font-semibold">{totalSprintRemaining}h</span> remaining
                    {totalSprintEstimated > 0 && <span className="ml-1">/ {totalSprintEstimated}h total</span>}
                  </span>
                ) : (
              ...
              <div className="w-full h-2 rounded-full bg-border overflow-hidden">
                <div
                  ...
                  style={{
                    width: showHours
                      ? totalSprintEstimated > 0 ? `${Math.round((totalSprintLogged / totalSprintEstimated) * 100)}%` : "0%"
          ...
          {sorted.length > 0 ? (
            <div className="space-y-3">
              {sorted.map((story) => {
                ...
                const totalEstimated = storyTasks.reduce((sum, t) => sum + (t.estimated_hours ?? 0), 0);
                const totalLogged = storyTasks.reduce((sum, t) => sum + (t.logged_hours ?? 0), 0);
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2 mb-2">
                          ...
                          <div className="flex items-center gap-1.5 flex-shrink-0">
                            {storyTasks.length > 0 && (
                              <span className="text-xs text-muted bg-background border border-border px-2 py-0.5 rounded-lg">{storyTasks.length} tasks</span>
                            )}
                            {story.story_points != null && (
                              <span className="text-xs text-muted bg-background border border-border px-2 py-0.5 rounded-lg">{story.story_points} pts</span>
                            )}
                            {totalEstimated > 0 && (
                        ...
                        {storyActionError && (
                          <p className="text-xs text-error mt-1">{storyActionError}</p>
                        )}
                      </div>
                    </div>
                        ...
                        {storyTasks.length > 0 ? (
                              ...
                              return (
                                <div key={category}>
                                  <div className="flex items-center gap-2 mb-2">
                                    ...
                                    <span className="text-[10px] font-bold tracking-widest uppercase text-muted">{cfg.label} ({tasks.length})</span>
                                  </div>
                                        ...
                                        <div className="flex-1 min-w-0">
                                          ...
                                          <div className="flex items-center gap-3 mt-1.5 text-xs text-muted flex-wrap">
                                            ...
                                            {canAddTasks && !task.is_accepted && task.status !== "completed" && (
                                              <>
                                                <button onClick={(e) => { e.stopPropagation(); openEdit(task); }} className="px-2 py-0.5 text-xs font-semibold rounded-lg transition-colors bg-primary-light text-primary border border-primary-border hover:bg-primary/20">Edit</button>
                                                <button onClick={(e) => { e.stopPropagation(); deleteTask(task.id); }} className="px-2 py-0.5 text-xs font-semibold rounded-lg transition-colors bg-error-light text-error border border-error-border hover:bg-error/20">Delete</button>
                                              </>
                                            )}
                                          </div>
                                        </div>
                          ...
                          <div className="text-center py-4 text-muted text-sm">
                            ...
                            {canAddTasks && story.status !== "done" && (
                              <button onClick={() => setSelectedStory(story)} className="ml-2 text-primary hover:text-primary-hover transition-colors font-medium">Add task →</button>
                            )}
                          </div>
                        )}
                      </div>
            ...
            <div className="p-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <textarea value={editDescription} onChange={(e) => setEditDescription(e.target.value)} rows={3} className="mt-1 block w-full px-3 py-2.5 rounded-lg text-sm bg-background border border-border text-foreground placeholder:text-subtle focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary resize-none" />
                </div>
                <div>
                  <label className="block text-xs font-semibold tracking-widest uppercase text-primary mb-1">Estimated hours</label>
                  <input type="number" min="0.1" step="0.5" value={editHours} onChange={(e) => setEditHours(e.target.value)} className="mt-1 block w-full px-3 py-2.5 rounded-lg text-sm bg-background border border-border text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary" />
                </div>
                <div>
                  ...
                  <select value={editAssigneeId} onChange={(e) => setEditAssigneeId(e.target.value)} className="mt-1 block w-full px-3 py-2.5 rounded-lg text-sm bg-background border border-border text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary">
                    <option value="">— No assignment —</option>
                    {editLoadingMembers ? <option disabled>Loading...</option> : editDevelopers.map((m) => (
                      <option key={m.user_id} value={m.user_id}>{m.user?.first_name} {m.user?.last_name} ({m.user?.email})</option>
                    ))}
                  </select>
                </div>
                {editError && (
              ...
              <div className="border-t border-border pt-4 mt-4 flex justify-end gap-3">
                <button onClick={() => setEditingTask(null)} className="px-5 py-2.5 text-sm font-medium rounded-lg transition-colors bg-background hover:bg-border text-muted">Cancel</button>
                <button onClick={saveEdit} disabled={editSaving} className="px-5 py-2.5 text-sm font-semibold text-white rounded-lg transition-colors shadow-sm disabled:opacity-50 bg-primary hover:bg-primary-hover">
                  {editSaving ? (
                    <span className="flex items-center gap-2">
                      <svg className="animate-spin h-4 w-4" viewBox="0 0 24 24" fill="none">
                        <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
      ...
      {selectedStory && (
        <StoryDetailModal
          ...
          onClose={() => { setSelectedStory(null); if (stories.length > 0) fetchTasks(stories.map((s) => s.id)); }}
          story={selectedStory}
          projectId={projectId}
