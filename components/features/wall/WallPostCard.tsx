...
function Avatar({ firstName, lastName, size = "md" }: { firstName?: string; lastName?: string; size?: "sm" | "md" }) {
  ...
  return (
    <span className={`${size === "sm" ? "w-6 h-6 text-[9px]" : "w-7 h-7 text-[10px]"} rounded-full bg-primary-light text-primary border border-primary-border flex items-center justify-center font-bold flex-shrink-0`}>
      {initials}
    </span>
  );
}

export default function WallPostCard({ id, author, content, created_at, projectId, isScrumMaster, onDeleted, initialCommentCount = 0 }: WallPostCardProps) {
  ...
  const loadComments = async () => {
    ...
    try {
      const res = await fetch(`/api/projects/${projectId}/wall/${id}/comments`, { credentials: "include" });
  ...
  const handleComment = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/projects/${projectId}/wall/${id}/comments`, {
        method: "POST",
  ...
  const handleDeleteComment = async (commentId: string) => {
    ...
    try {
      const res = await fetch(`/api/projects/${projectId}/wall/${id}/comments/${commentId}`, {
        method: "DELETE",
        credentials: "include",
      });
      ...
      <div className="flex-1 rounded-2xl border border-border bg-surface hover:border-subtle transition-colors">
        ...
        <div className="p-4">
            ...
            {isScrumMaster && (
              confirmDeletePost ? (
                <div className="flex items-center gap-1.5">
                  <span className="text-xs text-muted">Delete post + all comments?</span>
                ...
                <button onClick={() => setConfirmDeletePost(true)}
          ...
          >
            ...
            {expanded ? "Hide comments" : `${loaded ? comments.length : initialCommentCount} comment${(loaded ? comments.length : initialCommentCount) === 1 ? "" : "s"}`}
        ...
        {expanded && (
          <div className="border-t border-border bg-background rounded-b-2xl px-4 py-3 space-y-3">
            {loadingComments ? (
              ...
              <div className="space-y-3">
                {comments.map((comment) => (
                  <div key={comment.id} className="flex gap-2.5">
                    <Avatar firstName={comment.author?.first_name} lastName={comment.author?.last_name} size="sm" />
              ...
              <p className="text-xs text-muted py-1">No comments yet. Be the first!</p>
            )}
