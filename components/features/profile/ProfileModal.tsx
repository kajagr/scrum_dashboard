...
const labelClass = "block text-xs font-semibold tracking-widest uppercase text-primary mb-1";
...
export default function ProfileModal({ isOpen, onClose }: ProfileModalProps) {
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      ...
      try {
        ...
        if (!res.ok) { setErrors({ general: data.error || "Error loading profile." }); return; }
        setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { setErrors({ general: data.error || "Error saving changes." }); setLoading(false); return; }
      setLoading(false);
      onClose();
      router.refresh();
    } catch {
        ...
        <div className="relative w-full max-w-md mx-4 rounded-2xl overflow-hidden shadow-2xl bg-surface">
          ...
          <div className="p-7">
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                <p className="text-xs font-semibold tracking-widest uppercase text-primary mb-0.5">Account</p>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex items-center gap-6 mb-2">
                  ...
                  <div>
                    ...
                    <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 text-xs font-medium rounded-full border mt-0.5 ${
                      formData.role === "admin"
                        ? "bg-accent-light text-accent-text border-accent-border"
                        : "bg-surface text-muted border-border"
                    }`}>
                      <span className={`w-1.5 h-1.5 rounded-full ${formData.role === "admin" ? "bg-accent" : "bg-subtle"}`} />
                ...
                <div className="mt-5">
                  ...
                  <input type="text" name="username" value={formData.username} onChange={handleChange} autoFocus className={inputClass(!!errors.username)} />
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="text" name="firstName" value={formData.firstName} onChange={handleChange} className={inputClass(!!errors.firstName)} />
                  ...
                  <div>
                    ...
                    <input type="text" name="lastName" value={formData.lastName} onChange={handleChange} className={inputClass(!!errors.lastName)} />
