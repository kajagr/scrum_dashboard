...
export default function ChangePasswordModal({ isOpen, onClose }: ChangePasswordModalProps) {
  ...
  const validate = (): FieldErrors => {
    ...
    else if (newPassword.length < 12) e.newPassword = "Password must be at least 12 characters.";
    else if (newPassword.length > 128) e.newPassword = "Password cannot be longer than 128 characters.";
    if (!confirmPassword) e.confirmPassword = "Please confirm your new password.";
    else if (newPassword && confirmPassword !== newPassword) e.confirmPassword = "Passwords do not match.";
    return e;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 401) setErrors({ oldPassword: "Incorrect current password." });
      ...
      <div className="relative w-full max-w-md mx-4 rounded-2xl overflow-hidden shadow-2xl bg-surface">
        ...
        <div className="p-7">
          <div className="flex items-center justify-between mb-6">
            <div>
              <p className="text-xs font-semibold tracking-widest uppercase text-primary mb-0.5">Security</p>
              <h2 className="text-2xl font-bold text-foreground">Change Password</h2>
            </div>
            <button onClick={handleClose} className="w-8 h-8 flex items-center justify-center rounded-full text-lg transition-colors bg-background hover:bg-border text-muted">×</button>
          </div>
          ...
          {success ? (
            <div className="flex flex-col items-center justify-center py-8 gap-3">
              ...
              <p className="text-sm font-semibold text-foreground">Password changed successfully!</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    value={oldPassword}
                    onChange={(e) => { setOldPassword(e.target.value); setErrors((p) => ({ ...p, oldPassword: undefined })); }}
                    autoFocus
                    placeholder="Enter current password"
                    className={inputClass(!!errors.oldPassword)}
                  />
                  <button type="button" onClick={() => setShowOld((s) => !s)} className="absolute right-3 top-1/2 -translate-y-1/2 text-subtle hover:text-muted transition-colors">
                    <EyeIcon show={showOld} />
                  </button>
                </div>
                {errors.oldPassword && <p className="text-xs text-error mt-1">{errors.oldPassword}</p>}
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  />
                  <button type="button" onClick={() => setShowNew((s) => !s)} className="absolute right-3 top-1/2 -translate-y-1/2 text-subtle hover:text-muted transition-colors">
                  ...
                  <div className="mt-2 flex gap-1">
                    {[12, 16, 20].map((threshold, i) => (
                      <div key={i} className={`h-1 flex-1 rounded-full transition-colors ${
                        newPassword.length >= threshold ? "bg-primary" : "bg-border"
                      }`} />
                    ))}
                    <span className="text-[10px] text-subtle ml-2">
                      {newPassword.length < 12 ? "Too weak" : newPassword.length < 16 ? "Good" : newPassword.length < 20 ? "Strong" : "Very strong"}
                    </span>
                  </div>
                )}
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  />
                  <button type="button" onClick={() => setShowConfirm((s) => !s)} className="absolute right-3 top-1/2 -translate-y-1/2 text-subtle hover:text-muted transition-colors">
