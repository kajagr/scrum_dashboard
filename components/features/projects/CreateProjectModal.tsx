...
export default function CreateProjectModal({
  ...
  // shranjujemo cel objekt, ne samo id
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      fetch("/api/users/profile", { credentials: "include" })
        .then((r) => r.json())
        .then((d) => {
          if (d.id) setCurrentUser(d as User);
        })
        .catch(() => {});
    }
  }, [isOpen]);
  ...
  const handleNext = () => {
    ...
    // dodaj kreatorja kot product_owner takoj ob prehodu na step 2
    if (
      currentUser &&
      !selectedMembers.find((m) => m.user_id === currentUser.id)
    ) {
      setSelectedMembers([{ user_id: currentUser.id, role: "product_owner" }]);
    }
  ...
  const handleRoleChange = (userId: string, newRole: ProjectRole) => {
    if (UNIQUE_ROLES.includes(newRole)) {
      ...
      if (alreadyTaken) {
        const takenBy = getUserById(alreadyTaken.user_id);
        setError(
  ...
  const handleSubmit = async () => {
    const poCount = selectedMembers.filter(
      (m) => m.role === "product_owner",
    ).length;
    const smCount = selectedMembers.filter(
      (m) => m.role === "scrum_master",
    ).length;
    if (smCount === 0) {
      setError("You must assign a Scrum Master before creating the project.");
      return;
    }
    if (smCount > 1) {
      setError("Only one Scrum Master is allowed per project.");
      return;
    }
    if (poCount === 0) {
      setError("You must assign a Product Owner before creating the project.");
      return;
    }
    if (poCount > 1) {
      setError("Only one Product Owner is allowed per project.");
      return;
    }
    ...
    try {
      ...
      if (!projRes.ok) {
        ...
        else setError(projData.error || "Failed to create project.");
        setSubmitting(false);
        return;
      }
      ...
      if (selectedMembers.length > 0) {
        ...
        if (!membersRes.ok) {
          ...
          setError(
            `Project created, but adding members failed: ${membersData.error}`,
          );
          setSubmitting(false);
  ...
  // getUserById upošteva tudi kreatorja (ki ni v availableUsers)
  const getUserById = (id: string): User | undefined =>
    availableUsers.find((u) => u.id === id) ??
    (currentUser?.id === id ? currentUser : undefined);

  const filteredUsers = availableUsers
    .filter((u) => u.id !== currentUser?.id) // skrij kreatorja iz available
            ...
            {([1, 2] as const).map((s, i) => (
              <div key={s} className="flex items-center gap-3">
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`text-xs font-medium ${step === s ? "text-foreground" : "text-muted"}`}
                  >
                ...
                {i < 1 && (
                  <div
                    className={`w-6 h-px ${step > 1 ? "bg-primary" : "bg-border"}`}
                  />
                )}
              </div>
            ))}
            ...
            <div className="space-y-4 flex-1">
              <div>
                <label className={labelClass}>
                  Project name{" "}
                  <span className="text-accent normal-case font-normal tracking-normal">
                    *
                  </span>
                </label>
                <input
          ...
          {step === 2 && (
            <div className="flex flex-col gap-4 overflow-visible flex-1">
              ...
              <div className="flex gap-2 flex-shrink-0">
                {(["product_owner", "scrum_master"] as ProjectRole[]).map(
                  (role) => {
                ...
                <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-border bg-background text-xs text-subtle">
                  {selectedMembers.filter((m) => m.role === "developer").length}{" "}
                  developer
                  {selectedMembers.filter((m) => m.role === "developer")
                    .length !== 1
                    ? "s"
                    : ""}
                </div>
              ...
              <div className="flex-shrink-0">
                <p className={labelClass + " mb-2"}>
                  ...
                  {filteredUsers.length > 0 && (
                    <span className="ml-2 normal-case font-normal tracking-normal text-subtle">
