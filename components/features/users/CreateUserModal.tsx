...
export default function CreateUserModal({ isOpen, onClose }: CreateUserModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (Object.keys(validationErrors).length > 0) { setErrors(validationErrors); setLoading(false); return; }
    try {
      ...
      if (!res.ok) { setErrors({ general: data.error || "Error creating user." }); setLoading(false); return; }
      resetForm();
      onClose();
    } catch {
