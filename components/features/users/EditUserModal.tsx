...
const ROLES = [
  { value: "user" as const, label: "User", desc: "Regular member", icon: "M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" },
...
export default function EditUserModal({ isOpen, onClose, user }: EditUserModalProps) {
  ...
  const validateForm = (): FormErrors => {
    ...
    if (formData.password) {
      if (formData.password.length < 12) e.password = "Password must be at least 12 characters.";
      else if (formData.password.length > 64) e.password = "Password cannot be longer than 64 characters.";
    }
    return e;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setErrors({ general: data.error || "Error updating user." });
        return;
      }
      handleClose();
    } catch {
        ...
        <div className="p-7">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <h2 className="text-2xl font-bold text-foreground leading-tight">Edit user</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input type="text" name="username" value={formData.username} onChange={handleChange} autoFocus placeholder="e.g. janez123" className={inputClass(!!errors.username)} />
            ...
            <div>
              ...
              <input type="email" name="email" value={formData.email} onChange={handleChange} placeholder="john.doe@email.com" className={inputClass(!!errors.email)} />
              ...
              <div className="mt-1.5 grid grid-cols-2 gap-2">
                {ROLES.map((role) => (
                  <button
                    key={role.value}
                    type="button"
                    onClick={() => setFormData((p) => ({ ...p, systemRole: role.value }))}
                    className={`flex items-center gap-2.5 px-3 py-2.5 rounded-xl border text-left transition-all ${
                      formData.systemRole === role.value
                        ? role.value === "admin"
                          ? "bg-accent-light text-accent-text border-accent-border ring-1 ring-accent"
                          : "bg-primary-light text-primary border-primary-border ring-1 ring-primary"
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] opacity-70 leading-tight">{role.desc}</p>
                    </div>
                  </button>
                ))}
              </div>
            ...
            {/* Password (optional) */}
            <div>
              <label className={labelClass}>New password <span className="text-subtle normal-case font-normal tracking-normal">(optional)</span></label>
              <div className="relative mt-1">
                <input
                  type="text"
                  name="password"
                  value={showPassword ? formData.password : formData.password.split("").map((char, i) => (i === revealIndex ? char : "•")).join("")}
                  ...
                  className={`block w-full px-3 py-2.5 rounded-lg text-sm transition-all bg-background border text-foreground placeholder:text-subtle focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary pr-10 ${
                    errors.password ? "border-error-border focus:ring-error/20 focus:border-error" : "border-border"
                  }`}
                />
                <button type="button" onClick={() => setShowPassword((s) => !s)} className="absolute right-3 top-1/2 -translate-y-1/2 text-subtle hover:text-muted transition-colors">
