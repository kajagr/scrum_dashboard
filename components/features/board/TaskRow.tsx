...
export default function TaskRow({
  ...
  const handleStart = async () => {
    ...
    try {
      const res = await fetch(`/api/tasks/${task.id}/start`, { method: "POST", credentials: "include" });
      if (res.ok) onRefresh();
      else alert((await res.json()).error || "Napaka pri začetku dela.");
    } catch { alert("Napaka pri začetku dela."); }
  ...
  const handleStop = async () => {
    setActionLoading(true);
    try {
      const res = await fetch(`/api/tasks/${task.id}/stop`, { method: "POST", credentials: "include" });
      if (res.ok) onRefresh();
      else alert((await res.json()).error || "Napaka pri končanju dela.");
    } catch { alert("Napaka pri končanju dela."); }
    finally { setActionLoading(false); }
  };
  ...
  return (
    ...
    >
      {/* Checkbox */}
      <button
        onClick={() => onUpdateStatus(task.id, isDone ? "assigned" : "completed")}
        ...
        {isMyTask && !isDone && (
          resignConfirm ? (
            <div className="flex items-center gap-1">
              <button
                onClick={() => { setResignConfirm(false); onResign(task.id); }}
                disabled={loading}
