...
} from "@/components/features/board/TaskRow";
...
export default function StoryDetailModal({
  ...
  const deleteTask = async (taskId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert((await res.json()).error || "Error removing task.");
      }
    } finally {
      setUpdatingTaskId(null);
    }
  ...
  const canAccept =
    currentUserRole === "developer" || currentUserRole === "scrum_master";
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-2 flex-wrap">
                  <span
                    className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium ${priority.pill}`}
                  >
                    <span
                  ...
                  <span
                    className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${status.pill}`}
                  >
                    {status.label}
                  </span>
                </div>
            ...
            <div>
              ...
              {loadingTasks ? (
                          ...
                          {categoryTasks.map((task) => (
                            ...
                            >
                              ...
                              <button
                                ...
                                className="shrink-0 rounded-lg bg-red-600 px-3 py-2 text-xs font-semibold text-white transition-colors hover:bg-red-700 disabled:opacity-50"
                              >
                ...
                <div className="flex flex-col items-center justify-center py-10 rounded-xl border border-border bg-background text-center">
                  ...
                  <p className="text-xs text-muted mb-3">
                    Break this story into smaller tasks
                  </p>
              ...
              {story.status === "done" && (
                <div className="mt-4 flex items-center gap-2.5 p-3 rounded-xl border border-[rgba(52,211,153,0.25)] bg-[rgba(52,211,153,0.08)]">
                  <svg
                    className="w-4 h-4 text-[#34D399] flex-shrink-0"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    strokeWidth={2}
                  ...
                  <p className="text-xs text-[#34D399] font-medium">
                    This story is completed. No new tasks can be added.
                  </p>
                </div>
              )}
            </div>
