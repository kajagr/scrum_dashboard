...
export default function CreateStoryModal({
  ...
  const validate = (): boolean => {
    ...
    if (businessValue === "") {
      ...
      !Number.isInteger(Number(businessValue)) ||
      Number(businessValue) < 1 ||
      Number(businessValue) > 10
    ) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 409) {
          setFieldErrors({
            duplicate: `A story with the title "${title.trim()}" already exists in this project.`,
          });
        } else if (res.status === 403) {
          setServerError(
            "You do not have permission to add user stories. Only the Product Owner and Scrum Master can add stories.",
          );
            ...
            <div>
              ...
              <input
                ...
                className={inputClass(
                  !!fieldErrors.title || !!fieldErrors.duplicate,
                )}
              />
              ...
              <div className="mt-1.5 grid grid-cols-2 gap-2">
                {PRIORITIES.map((p) => (
                  <button
                    ...
                    onClick={() => {
                      setPriority(p.value);
                      setFieldErrors((f) => ({ ...f, priority: undefined }));
                    }}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  setFieldErrors((f) => ({ ...f, businessValue: undefined }));
                }}
                placeholder="1 – 10"
                className={inputClass(!!fieldErrors.businessValue)}
              />
              {fieldErrors.businessValue && (
                <p className="text-xs text-error mt-1">
                  {fieldErrors.businessValue}
                </p>
              )}
